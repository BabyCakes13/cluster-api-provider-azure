...
# Wait a moment for Tilt to start
sleep 5

# Check if Tilt started successfully by looking for expected output
TILT_SUCCESS=0
for i in {1..30}; do
    if ps -p $TILT_PID > /dev/null; then
        # Check if the Tilt server is actually running
        if curl -s http://localhost:10350/ > /dev/null; then
            echo "Tilt has started successfully on http://localhost:10350/"
            TILT_SUCCESS=1
            break
        fi
    else
        echo "Tilt process has ended unexpectedly."
        break
    fi
    echo "Waiting for Tilt to start... ($i/30)"
    sleep 2
done

# If Tilt started successfully, update tilt-settings.yaml
if [ $TILT_SUCCESS -eq 1 ]; then
    echo "Updating tilt-settings.yaml with custom settings..."
    
    # Check if tilt-settings.yaml exists
    if [ -f "./tilt-settings.yaml" ]; then
        # Update the kustomize_substitutions section with SUBSCRIPTION_TYPE: "corporate"
        # First check if kustomize_substitutions section already exists
        if grep -q "kustomize_substitutions:" ./tilt-settings.yaml; then
            # Check if SUBSCRIPTION_TYPE already exists in the kustomize_substitutions section
            if grep -q "SUBSCRIPTION_TYPE:" ./tilt-settings.yaml; then
                # Replace existing SUBSCRIPTION_TYPE value
                sed -i 's/SUBSCRIPTION_TYPE:.*$/SUBSCRIPTION_TYPE: "corporate"/' ./tilt-settings.yaml
            else
                # Add SUBSCRIPTION_TYPE to existing kustomize_substitutions section
                sed -i '/kustomize_substitutions:/a\  SUBSCRIPTION_TYPE: "corporate"' ./tilt-settings.yaml
            fi
        else
            # Add new kustomize_substitutions section with SUBSCRIPTION_TYPE
...
EOF
        ...
        echo "Successfully updated tilt-settings.yaml with SUBSCRIPTION_TYPE: corporate"
    else
        echo "Warning: tilt-settings.yaml not found. Creating new file..."
        cat > ./tilt-settings.yaml << EOF
kustomize_substitutions:
  SUBSCRIPTION_TYPE: "corporate"
EOF
        echo "Created new tilt-settings.yaml with SUBSCRIPTION_TYPE: corporate"
    fi
    
    echo "tilt-settings.yaml current content:"
    cat ./tilt-settings.yaml
    
    echo "You can now access Tilt at http://localhost:10350/"
    echo "Press Ctrl+C when you want to stop Tilt"
    
    # Keep the script running until user terminates it
    wait $TILT_PID
else
    echo "ERROR: Tilt did not start successfully within the timeout period."
